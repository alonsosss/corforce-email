...
| `check-backups.sh` | Respaldos sin docker: con un `docker` simulado, que en el perfil autoalojado las herramientas de Postgres corran en contenedor de la imagen del Postgres en marcha, en su red, con `verify-full`, la CA montada y la contraseña solo por nombre, y que en `aws` (y sin `.env`) se usen los binarios del host sin tocar `PGSSLMODE`; que ningún guion de `ops/backup` llame a `psql`, `pg_dump` o `pg_restore` por su cuenta; que el respaldo cubra las tres clases de base y la verificación exija registro y celda; copia externa con el CLI fijado por digest, sin credenciales ni frase en argumentos y con sus secretos fuera de las listas que reciben los contenedores y de `.env.example`; volúmenes de correo (imagen de `tar` fijada e igual en respaldo y restauración, `crypt-vol` nunca sin cifrar fuera, `_garbage` excluido); unidades de systemd coherentes con `install-timers.sh` (`User`, `UMask=0077`, `ExecStart`, `APP_DIR`, todos los temporizadores) y `bootstrap.sh` instalando por ese guion; una alerta por cada trabajo que publique métrica; y `ops/backup`, `ops/db` y `ops/maintenance` sincronizados al servidor; lo corre `validate.sh` |
