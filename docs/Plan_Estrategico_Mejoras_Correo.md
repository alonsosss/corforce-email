...
| C2 Respuesta automática | Parcial | Backend, Dovecot y pruebas hechos (V, 2026-09-21): `mail.vacation_replies` y `v_sieve_vacation` (`09_vacation.sql`), API de administración e interna para el webmail en `mail-directory`, tercera ranura `sieve_after3` en Dovecot, y `make e2e-mail` con Dovecot real (fuera de la ventana no contesta, dentro contesta, cabecera de respuesta automática, texto literal). Pendiente: la pantalla en el webmail y en la ficha del buzón, y la ruta del servicio `webmail` (P). Nota: ManageSieve ya permitía la respuesta automática a quien use un cliente que lo soporte; lo nuevo es hacerlo desde nuestras pantallas |
| B6, B7, A2 a A6, C1, C3 a C5 | P | Sin implementar |
