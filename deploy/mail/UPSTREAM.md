...
| `dovecot/conf/dovecot.conf` | postgres, sogo-php, migracion | Los diccionarios de cuota y sieve son pgsql, no mysql; sin ejemplo LDAP ni SOGo; el passwd-verify.lua renderizado vive fuera del bind mount (/etc/dovecot-auth); incluye doveadm-api.conf generado; ademas declara el diccionario `sieve_vacation` y la ranura `sieve_after3` de la respuesta automatica (`mail.v_sieve_vacation`), y el diccionario `sieve_user` y la ranura `sieve_before3` de las reglas y el reenvio del webmail (`mail.v_sieve_user`); y una passdb Lua antes de las demas (`migration-verify.lua`, la credencial de destino por trabajo de migracion) con `cache_key=%s:%u:%w:%{session}` | Reaplicar el cambio de mailcow y volver a poner las lineas propias. Conservar los diccionarios, `sieve_before3`, `sieve_after3` y la passdb de migracion, que debe ir la primera. |
...
| `dovecot/docker-entrypoint.sh` | postgres, sogo-php, servicios-go, nombres, migracion | Genera el userdb y los dicts de Dovecot contra PostgreSQL; quita SOGo (IP de confianza, SSO, credenciales de cron); renderiza passwd-verify.lua y migration-verify.lua; conserva el usuario maestro del webmail bajo @platform.local; genera la API HTTP de doveadm con TLS. Es de los ficheros que mas cuesta portar; ademas genera `dovecot-dict-sql-sieve_vacation.conf` (la respuesta automatica) y `dovecot-dict-sql-sieve_user.conf` (las reglas y el reenvio del webmail) y, solo con `DOVECOT_MIGRATION_MASTER_USER` y `DOVECOT_MIGRATION_MASTER_PASS`, un segundo usuario maestro propio de la migracion (su linea en `dovecot-master.passwd` con su `allow_nets` y en `dovecot-master.userdb`); y da permiso de ejecucion a `maildir_reconcile.sh` junto al resto de guiones | Aplicar los cambios de mailcow bloque a bloque, no el fichero entero, y comprobar con make e2e-mail. Conservar los bloques `sieve_vacation` y `sieve_user`, el del maestro de la migracion, el renderizado de `migration-verify.lua` y `maildir_reconcile.sh` en la lista del `chmod +x`. |
...
| `postfix/postfix.sh` | postgres, nombres, sogo-php | Genera los 18 mapas pgsql_*.cf contra el esquema mail en lugar de los de MySQL, sin BCC dinamicos ni SOGo, y con los nombres propios. Es el fichero que mas cuesta portar. `pgsql_relay_ne.cf` mira ademas `mail.mailboxes`: aqui un buzon no deja una fila de alias con su propia direccion, asi que con la consulta de origen todo buzon ya migrado de un dominio en coexistencia se reenviaba al proveedor anterior en vez de entregarse en local. | Aplicar los cambios de mailcow bloque a bloque, no el fichero entero, y comprobar los 18 mapas con postmap -q (make e2e-mail), incluidos los tres casos de `pgsql_relay_ne`: alias, buzon migrado y direccion sin migrar. |
...
| `rspamd/local.d/mx_check.conf` | endurecimiento | `exclude_mxs` con `custom/own_mx.map` (el `MAIL_HOSTNAME`, lo escribe el arranque): desde un contenedor no se alcanza el puerto 25 de la IP publica del mismo host y el correo desde los dominios de las empresas se penalizaba con `MX_TIMEOUT_CONNECT`; con el mapa da `MX_WHITE` sin sondear. | Conservar la linea y el mapa del arranque. |
