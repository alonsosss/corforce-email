...
--
-- Rol de servicio del esquema automations en la base de CADA empresa. Hasta ahora automations abria
...
-- guardan los demas. Con este rol, automations solo llega a automations.
--
-- Dos roles, el mismo patron que la celda (mail-directory/06_service_role.sql): el grupo
-- NOLOGIN automations_service tiene los permisos -que se enumeran AQUI, junto a las tablas que
-- los necesitan- y el rol de login mail_svc_automations, que crea ops/db/tenant-service-role.sh
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'automations_service') THEN
        CREATE ROLE automations_service NOLOGIN;
    END IF;
END $$;

GRANT USAGE ON SCHEMA automations TO automations_service;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA automations TO automations_service;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA automations TO automations_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA automations
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO automations_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA automations
    GRANT USAGE, SELECT ON SEQUENCES TO automations_service;
...
BEGIN
    IF to_regclass('platform.event_outbox') IS NOT NULL THEN
        EXECUTE 'GRANT USAGE ON SCHEMA platform TO automations_service';
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON platform.event_outbox TO automations_service';
    END IF;
END $$;
...
-- PUBLIC y le aplica las canonicas, y esta le da la entrada al rol de automations y a nadie mas.
DO $$
BEGIN
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO automations_service', current_database());
END $$;
