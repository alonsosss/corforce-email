...
--
-- Rol de servicio del esquema templates en la base de CADA empresa. Hasta ahora templates abria
...
-- guardan los demas. Con este rol, templates solo llega a templates.
--
-- Dos roles, el mismo patron que la celda (mail-directory/06_service_role.sql): el grupo
-- NOLOGIN templates_service tiene los permisos -que se enumeran AQUI, junto a las tablas que
-- los necesitan- y el rol de login mail_svc_templates, que crea ops/db/tenant-service-role.sh
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'templates_service') THEN
        CREATE ROLE templates_service NOLOGIN;
    END IF;
END $$;

GRANT USAGE ON SCHEMA templates TO templates_service;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA templates TO templates_service;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA templates TO templates_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA templates
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO templates_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA templates
    GRANT USAGE, SELECT ON SEQUENCES TO templates_service;
...
BEGIN
    IF to_regclass('platform.event_outbox') IS NOT NULL THEN
        EXECUTE 'GRANT USAGE ON SCHEMA platform TO templates_service';
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON platform.event_outbox TO templates_service';
    END IF;
END $$;
...
-- PUBLIC y le aplica las canonicas, y esta le da la entrada al rol de templates y a nadie mas.
DO $$
BEGIN
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO templates_service', current_database());
END $$;
