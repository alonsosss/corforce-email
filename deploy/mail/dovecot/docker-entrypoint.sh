...
# Reglas y reenvio de cada buzon: los genera mail-directory desde el webmail y los lee la vista
# v_sieve_user, en su propia ranura (sieve_before3).
cat <<EOF > /etc/dovecot/sql/dovecot-dict-sql-sieve_user.conf
# Autogenerated by Core Force Mail
connect = "${DOVECOT_PG_CONNECT}"
map {
  pattern = priv/sieve/name/\$script_name
  table = mail.v_sieve_user
  username_field = username
  value_field = id
  fields {
    script_name = \$script_name
