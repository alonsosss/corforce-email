...
| Despliegue | Hecho (2026-09-23, `5f498e7` y `a9c3023`): `clamd-mail` con la red `mail-scan`; `organization` (registro 038 y empresa `templates/03` en las dos empresas); MinIO e inicializacion; `gateway`, `templates`, `mail-security` y `web`. Comprobado en produccion: subida analizada por ClamAV y guardada en MinIO, imagen servida por `/media/public/*` con cache inmutable, 304 y 404 fuera de `public/`, verificacion con puntuacion real de Rspamd (0,89 `no action` para un correo correcto). En el navegador (entorno local con el mismo codigo): kit de marca, galeria, editor, panel de revision y publicacion con el diseno y los comentarios de Outlook guardados, sin violaciones de CSP |
