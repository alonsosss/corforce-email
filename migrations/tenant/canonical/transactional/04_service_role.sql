...
--
-- Rol de servicio del esquema transactional en la base de CADA empresa. Hasta ahora transactional abria
...
-- guardan los demas. Con este rol, transactional solo llega a transactional.
--
-- Dos roles, el mismo patron que la celda (mail-directory/06_service_role.sql): el grupo
-- NOLOGIN transactional_service tiene los permisos -que se enumeran AQUI, junto a las tablas que
-- los necesitan- y el rol de login mail_svc_transactional, que crea ops/db/tenant-service-role.sh
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'transactional_service') THEN
        CREATE ROLE transactional_service NOLOGIN;
    END IF;
END $$;

GRANT USAGE ON SCHEMA transactional TO transactional_service;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA transactional TO transactional_service;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA transactional TO transactional_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA transactional
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO transactional_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA transactional
    GRANT USAGE, SELECT ON SEQUENCES TO transactional_service;
...
BEGIN
    IF to_regclass('platform.event_outbox') IS NOT NULL THEN
        EXECUTE 'GRANT USAGE ON SCHEMA platform TO transactional_service';
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON platform.event_outbox TO transactional_service';
    END IF;
END $$;
...
-- PUBLIC y le aplica las canonicas, y esta le da la entrada al rol de transactional y a nadie mas.
DO $$
BEGIN
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO transactional_service', current_database());
END $$;
