...
const (
	...
	ConsumerName          = "mail-dav-mailbox-deleted"
...
func (c *Consumer) Run(ctx context.Context) {
	...
	for {
		...
		if err == nil {
			c.logger.Info("mail-dav: suscrito a los buzones borrados", zap.String("consumer", ConsumerName))
			return
		}
		c.logger.Warn("mail-dav: no se pudo suscribir a los buzones borrados; se reintenta", zap.Error(err))
		select {
		case <-ctx.Done():
			return
...
func (c *Consumer) Handle(evt events.Event, ack func()) {
	...
	if tenantErr != nil || mailboxErr != nil || tenantID == uuid.Nil || mailboxID == uuid.Nil ||
		...
		c.logger.Error("mail-dav: evento de buzon borrado sin empresa y buzon coherentes; queda para EVENTS_DLQ",
