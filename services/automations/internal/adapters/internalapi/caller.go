...
// transactional, contacts y templates: token interno, empresa en cabecera, envelope de
...
// maxResponseBytes acota lo que se lee de un vecino.
...
// autoriza al cliente HTTP a repetir la peticion ante un fallo transitorio; header anade
// cabeceras propias de la llamada (Idempotency-Key).
func (c *Caller) Post(ctx context.Context, tenantID uuid.UUID, path string, body any, idempotent bool, header http.Header, out any) error {
	if c.baseURL == "" {
...
// Classify traduce la respuesta de un vecino a los errores de los puertos: 429 espera,
// 403 bloqueo, 400/404/409/422 rechazo definitivo; el resto es transitorio.
func Classify(err error) error {
	...
	case http.StatusBadRequest, http.StatusNotFound, http.StatusConflict, http.StatusUnprocessableEntity:
		...
		return &ports.RejectedError{Status: se.Status, Code: se.Code, Message: msg}
	}
