...
if [[ ! -z ${REDIS_SLAVEOF_IP} ]]; then
  export REDIS_CMDLINE="redis-cli -h ${REDIS_SLAVEOF_IP} -p ${REDIS_SLAVEOF_PORT} -a ${MAIL_REDIS_PASSWORD} --no-auth-warning"
else
  export REDIS_CMDLINE="redis-cli -h ${MAIL_REDIS_HOST} -p 6379 -a ${MAIL_REDIS_PASSWORD} --no-auth-warning"
fi
...
# Request certificate for MAIL_HOSTNAME only
if [[ "${ONLY_MAIL_HOSTNAME}" =~ ^([yY][eE][sS]|[yY])+$ ]]; then
  ONLY_MAIL_HOSTNAME=y
fi
...
if [[ -f ${ACME_BASE}/cert.pem ]] && [[ -f ${ACME_BASE}/key.pem ]] && [[ $(stat -c%s ${ACME_BASE}/cert.pem) != 0 ]]; then
  ...
  if [[ ${ISSUER} != *"Let's Encrypt"* && ${ISSUER} != *"mailcow"* && ${ISSUER} != *"Fake LE Intermediate"* ]]; then
    # "mailcow" is the issuer CN of the bundled ssl-example snake-oil certificate
    log_f "Found certificate with issuer other than the snake-oil CA and Let's Encrypt, skipping ACME client..."
    sleep 3650d
    exec $(readlink -f "$0")
  fi
else
  if [[ -f ${ACME_BASE}/${MAIL_HOSTNAME}/cert.pem ]] && [[ -f ${ACME_BASE}/${MAIL_HOSTNAME}/key.pem ]] && verify_hash_match ${ACME_BASE}/${MAIL_HOSTNAME}/cert.pem ${ACME_BASE}/${MAIL_HOSTNAME}/key.pem; then
    log_f "Restoring previous acme certificate and restarting script..."
    cp ${ACME_BASE}/${MAIL_HOSTNAME}/cert.pem ${ACME_BASE}/cert.pem
    cp ${ACME_BASE}/${MAIL_HOSTNAME}/key.pem ${ACME_BASE}/key.pem
    # Restarting with env var set to trigger a restart,
    exec env TRIGGER_RESTART=1 $(readlink -f "$0")
  else
    log_f "Restoring snake-oil certificates and restarting script..."
...
while ! pg_isready -q -h "${MAIL_DB_HOST}" -p "${MAIL_DB_PORT:-5432}" -U "${MAIL_DB_USER}" -d "${MAIL_DB_NAME}" > /dev/null; do
  sleep 2
done
log_f "Database OK"
...
# Waiting for domain table (created by the mail-directory migrations)
log_f "Waiting for domain table..."
while [[ -z ${DOMAIN_TABLE} ]]; do
  DOMAIN_TABLE=$(${PSQL} -c "SELECT to_regclass('mail.domains')" 2>/dev/null)
...
while true; do
  ...
  # Re-using previous acme account and domain keys
  ...
  SQL_DOMAINS=$(${PSQL} -c "SELECT domain FROM mail.domains WHERE NOT backupmx AND active")
  if [[ ! $? -eq 0 ]]; then
  ...
  if [[ ${ONLY_MAIL_HOSTNAME} != "y" ]]; then
  # MTA-STS: no hay tabla de politicas en el esquema mail todavia (pendiente del servicio
  # mail-directory), asi que ningun dominio pide certificado para mta-sts.<dominio>.
  ...
  for SQL_DOMAIN in "${SQL_DOMAIN_ARR[@]}"; do
    ...
    for SUBDOMAIN in "${ADDITIONAL_WC_ARR[@]}"; do
      ...
      # Skip if subdomain matches MAIL_HOSTNAME
      if [[ "${FULL_SUBDOMAIN}" == "${MAIL_HOSTNAME}" ]]; then
        continue
      fi
  ...
  if check_domain ${MAIL_HOSTNAME}; then
    VALIDATED_MAIL_HOSTNAME="${MAIL_HOSTNAME}"
  fi
  ...
  # Check if MAIL_HOSTNAME is covered by a wildcard in ADDITIONAL_SAN
  MAIL_HOSTNAME_COVERED=0
  if [[ ! -z ${VALIDATED_MAIL_HOSTNAME} ]]; then
    if is_covered_by_wildcard "${VALIDATED_MAIL_HOSTNAME}"; then
      MAIL_PARENT_DOMAIN=$(echo ${VALIDATED_MAIL_HOSTNAME} | cut -d. -f2-)
      log_f "MAIL_HOSTNAME '${VALIDATED_MAIL_HOSTNAME}' is covered by wildcard '*.${MAIL_PARENT_DOMAIN}' - skipping explicit hostname"
      MAIL_HOSTNAME_COVERED=1
    fi
  fi
  ...
  if [[ ${ENABLE_SSL_SNI} == "y" ]]; then
    ...
    if [[ ${MAIL_HOSTNAME_COVERED} == "1" ]]; then
      SERVER_SAN_VALIDATED=($(echo ${ADDITIONAL_VALIDATED_SAN[*]} | xargs -n1 | sort -u | xargs))
    else
      SERVER_SAN_VALIDATED=(${VALIDATED_MAIL_HOSTNAME} $(echo ${ADDITIONAL_VALIDATED_SAN[*]} | xargs -n1 | sort -u | xargs))
    fi
  else
    ...
    if [[ ${MAIL_HOSTNAME_COVERED} == "1" ]]; then
      ...
      SERVER_SAN_VALIDATED=(${VALIDATED_MAIL_HOSTNAME} $(echo ${VALIDATED_CONFIG_DOMAINS[*]} ${ADDITIONAL_VALIDATED_SAN[*]} | xargs -n1 | sort -u | xargs))
    fi
  fi
  ...
  if [[ -z ${VALIDATED_CERTIFICATES[*]} ]]; then
    ...
    log_f "Use SKIP_LETS_ENCRYPT=y in .env to skip it permanently."
