...
--
-- Rol de servicio del esquema suppression en la base de CADA empresa. Hasta ahora suppression abria
...
-- guardan los demas. Con este rol, suppression solo llega a suppression.
--
-- Dos roles, el mismo patron que la celda (mail-directory/06_service_role.sql): el grupo
-- NOLOGIN suppression_service tiene los permisos -que se enumeran AQUI, junto a las tablas que
-- los necesitan- y el rol de login mail_svc_suppression, que crea ops/db/tenant-service-role.sh
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'suppression_service') THEN
        CREATE ROLE suppression_service NOLOGIN;
    END IF;
END $$;

GRANT USAGE ON SCHEMA suppression TO suppression_service;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA suppression TO suppression_service;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA suppression TO suppression_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA suppression
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO suppression_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA suppression
    GRANT USAGE, SELECT ON SEQUENCES TO suppression_service;
...
BEGIN
    IF to_regclass('platform.event_outbox') IS NOT NULL THEN
        EXECUTE 'GRANT USAGE ON SCHEMA platform TO suppression_service';
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON platform.event_outbox TO suppression_service';
    END IF;
END $$;
...
-- PUBLIC y le aplica las canonicas, y esta le da la entrada al rol de suppression y a nadie mas.
DO $$
BEGIN
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO suppression_service', current_database());
END $$;
