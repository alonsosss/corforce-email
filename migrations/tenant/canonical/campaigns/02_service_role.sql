...
--
-- Rol de servicio del esquema campaigns en la base de CADA empresa. Hasta ahora campaigns abria
...
-- guardan los demas. Con este rol, campaigns solo llega a campaigns.
--
-- Dos roles, el mismo patron que la celda (mail-directory/06_service_role.sql): el grupo
-- NOLOGIN campaigns_service tiene los permisos -que se enumeran AQUI, junto a las tablas que
-- los necesitan- y el rol de login mail_svc_campaigns, que crea ops/db/tenant-service-role.sh
...
GRANT USAGE ON SCHEMA campaigns TO campaigns_service;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA campaigns TO campaigns_service;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA campaigns TO campaigns_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA campaigns
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO campaigns_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA campaigns
    GRANT USAGE, SELECT ON SEQUENCES TO campaigns_service;
...
BEGIN
    IF to_regclass('platform.event_outbox') IS NOT NULL THEN
        EXECUTE 'GRANT USAGE ON SCHEMA platform TO campaigns_service';
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON platform.event_outbox TO campaigns_service';
    END IF;
END $$;
...
-- PUBLIC y le aplica las canonicas, y esta le da la entrada al rol de campaigns y a nadie mas.
DO $$
BEGIN
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO campaigns_service', current_database());
END $$;
