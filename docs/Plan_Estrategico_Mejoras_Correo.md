...
| C1 Migracion de buzones | ADR (sin implementar) | `docs/adr/0002-migracion-de-buzones-con-imapsync.md` (2026-09-21): imapsync en un ejecutor aislado (sin base ni Redis, salida solo al origen) y un servicio `mail-migration` que guarda los trabajos; licencia de imapsync comprobada sobre su `LICENSE` (NO LIMIT PUBLIC LICENSE, sin restricciones). Preguntas abiertas: ClamAV sobre lo migrado y OAuth2 de Google y Microsoft (P: decision de producto) |
| A3 MTA-STS y TLS-RPT | ADR (sin implementar) | `docs/adr/0003-mta-sts-y-tls-rpt-entrantes.md` (2026-09-21): politica por dominio en `mail-directory`, con `testing` obligatorio de partida, servida dinamicamente y con los registros publicados por `domain-service`. Depende de que quien opera el servidor decida como atiende el borde y como se emiten los certificados de `mta-sts.<dominio>` (P). El panel de informes DMARC y TLS-RPT queda despues |
| A4, A5, A6, C3 fase 2, C5 | P | Sin implementar (por demanda o dependientes de trafico y de la observabilidad, segun el plan) |
