...
# mail-policy sirve los mapas dinamicos (8081) y recibe la exportacion de metadatos (9081)
until nc ${MAIL_POLICY_HOST:-mail-policy} 8081 -z; do
  echo "Waiting for mail-policy on port 8081..."
  sleep 3
done

until nc ${MAIL_POLICY_HOST:-mail-policy} 9081 -z; do
  echo "Waiting for mail-policy on port 9081..."
  sleep 3
done
...
# Contrasenas del controller desde el almacen de secretos, en cada arranque (controller-password.sh). Si
# falla, el controller queda cerrado fuera del bucle local (sin ninguna contrasena Rspamd responde 401) y
# Rspamd arranca igual: una contrasena mal puesta no puede dejar el correo sin filtrar.
if ! /controller-password.sh; then
  echo "controller-password: fallo; el controller queda cerrado hasta corregir el almacen de secretos" >&2
...
password = "${MAIL_REDIS_PASSWORD}";
timeout = 10;
EOF
  until [[ $(redis-cli -h ${MAIL_REDIS_HOST} -a ${MAIL_REDIS_PASSWORD} --no-auth-warning PING) == "PONG" ]]; do
    echo "Waiting for Redis @${MAIL_REDIS_HOST}..."
    sleep 2
  done
  until [[ $(redis-cli -h ${REDIS_SLAVEOF_IP} -p ${REDIS_SLAVEOF_PORT} -a ${MAIL_REDIS_PASSWORD} --no-auth-warning PING) == "PONG" ]]; do
    echo "Waiting for Redis @${REDIS_SLAVEOF_IP}..."
    sleep 2
  done
  redis-cli -h ${MAIL_REDIS_HOST} -a ${MAIL_REDIS_PASSWORD} --no-auth-warning SLAVEOF ${REDIS_SLAVEOF_IP} ${REDIS_SLAVEOF_PORT}
else
  cat <<EOF > /etc/rspamd/local.d/redis.conf
servers = "redis:6379";
password = "${MAIL_REDIS_PASSWORD}";
...
EOF
  ...
  redis-cli -h ${MAIL_REDIS_HOST} -a ${MAIL_REDIS_PASSWORD} --no-auth-warning SLAVEOF NO ONE
fi
...
# These are managed by the platform (mail-policy) and should not be removed
touch /etc/rspamd/custom/global_mime_from_blacklist.map \
  ...
  /etc/rspamd/custom/platform_networks.map \
...
# uid/gid 82 (the writer of custom maps, see README) needs to write to these files
...
# If DQS KEY is set in .env add Spamhaus DQS RBLs
if [[ ! -z ${SPAMHAUS_DQS_KEY} ]]; then
    cat <<EOF > /etc/rspamd/custom/dqs-rbl.conf
  # Autogenerated by Core Force Mail. DO NOT TOUCH!
    spamhaus {
