...
if [[ "$(cat /etc/rspamd/custom/sa-rules | md5sum | cut -d' ' -f1)" != "${HASH_SA_RULES}" ]]; then
  CONTAINER_NAME=rspamd-mail
  CONTAINER_ID=$(curl --silent --insecure https://dockerapi.mail-engines/containers/json | \
  ...
  if [[ ! -z ${CONTAINER_ID} ]]; then
    curl --silent --insecure -XPOST --connect-timeout 15 --max-time 120 https://dockerapi.mail-engines/containers/${CONTAINER_ID}/restart
  fi
fi
