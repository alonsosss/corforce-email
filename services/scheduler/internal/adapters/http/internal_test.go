...
func newInternalFixture(t *testing.T, status string) internalFixture {
	...
	tenant := uuid.New()
	now := time.Now().UTC()
	job := &domain.JobDefinition{ID: uuid.New(), TenantID: &tenant, Name: "Informe", Code: "informe", Handler: "reports.daily",
		IsActive: true, MaxRetries: 1, Timezone: domain.DefaultTimezone}
