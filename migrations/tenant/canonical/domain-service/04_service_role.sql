...
--
-- Rol de servicio del esquema domains en la base de CADA empresa. Hasta ahora domain-service abria
...
-- guardan los demas. Con este rol, domain-service solo llega a domains.
--
-- Dos roles, el mismo patron que la celda (mail-directory/06_service_role.sql): el grupo
-- NOLOGIN domains_service tiene los permisos -que se enumeran AQUI, junto a las tablas que
-- los necesitan- y el rol de login mail_svc_domains, que crea ops/db/tenant-service-role.sh
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'domains_service') THEN
        BEGIN
            CREATE ROLE domains_service NOLOGIN;
...
GRANT USAGE ON SCHEMA domains TO domains_service;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA domains TO domains_service;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA domains TO domains_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA domains
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO domains_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA domains
    GRANT USAGE, SELECT ON SEQUENCES TO domains_service;
...
BEGIN
    IF to_regclass('platform.event_outbox') IS NOT NULL THEN
        EXECUTE 'GRANT USAGE ON SCHEMA platform TO domains_service';
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON platform.event_outbox TO domains_service';
    END IF;
END $$;
...
-- PUBLIC y le aplica las canonicas, y esta le da la entrada al rol de domain-service y a nadie mas.
DO $$
BEGIN
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO domains_service', current_database());
END $$;
