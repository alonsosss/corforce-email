...
async def lifespan(app: FastAPI):
  ...
  if os.environ['REDIS_SLAVEOF_IP'] != "":
    redis_client = redis = await aioredis.from_url(f"redis://{os.environ['REDIS_SLAVEOF_IP']}:{os.environ['REDIS_SLAVEOF_PORT']}/0", password=os.environ['MAIL_REDIS_PASSWORD'])
  else:
    redis_client = redis = await aioredis.from_url(f"redis://{os.environ['MAIL_REDIS_HOST']}:6379/0", password=os.environ['MAIL_REDIS_PASSWORD'])
