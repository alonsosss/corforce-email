...
def refreshF2bregex():
  ...
  if not r.get('F2B_REGEX'):
    f2bregex = {}
    f2bregex[1] = r'Core Force Mail UI: Invalid password for .+ by ([0-9a-f\.:]+)'
...
if __name__ == '__main__':
  ...
  clear()
  logger.logInfo("Initializing platform netfilter chain")
  ...
  if os.getenv("DISABLE_NETFILTER_ISOLATION_RULE", "").lower() in ("y", "yes"):
    ...
    tables.create_mailcow_isolation_rule("br-mail", [3306, 6379, 8983, 12345], os.getenv("MAIL_REPLICA_IP"))
  ...
  while True:
    try:
      ...
      if "".__eq__(redis_slaveof_ip):
        r = redis.StrictRedis(
          host=os.getenv('IPV4_NETWORK', '172.22.1') + '.249', decode_responses=True, port=6379, db=0, password=os.environ['MAIL_REDIS_PASSWORD'])
      else:
        r = redis.StrictRedis(
          host=redis_slaveof_ip, decode_responses=True, port=redis_slaveof_port, db=0, password=os.environ['MAIL_REDIS_PASSWORD'])
      r.ping()
      pubsub = r.pubsub()
    except Exception as ex:
      logdebug(
