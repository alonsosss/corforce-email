...
--
-- Rol de servicio del esquema contacts en la base de CADA empresa. Hasta ahora contacts abria
...
-- guardan los demas. Con este rol, contacts solo llega a contacts.
--
-- Dos roles, el mismo patron que la celda (mail-directory/06_service_role.sql): el grupo
-- NOLOGIN contacts_service tiene los permisos -que se enumeran AQUI, junto a las tablas que
-- los necesitan- y el rol de login mail_svc_contacts, que crea ops/db/tenant-service-role.sh
...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'contacts_service') THEN
        BEGIN
            CREATE ROLE contacts_service NOLOGIN;
...
GRANT USAGE ON SCHEMA contacts TO contacts_service;
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA contacts TO contacts_service;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA contacts TO contacts_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA contacts
    GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO contacts_service;
ALTER DEFAULT PRIVILEGES IN SCHEMA contacts
    GRANT USAGE, SELECT ON SEQUENCES TO contacts_service;
...
BEGIN
    IF to_regclass('platform.event_outbox') IS NOT NULL THEN
        EXECUTE 'GRANT USAGE ON SCHEMA platform TO contacts_service';
        EXECUTE 'GRANT SELECT, INSERT, UPDATE, DELETE ON platform.event_outbox TO contacts_service';
    END IF;
END $$;
...
-- PUBLIC y le aplica las canonicas, y esta le da la entrada al rol de contacts y a nadie mas.
DO $$
BEGIN
    EXECUTE format('GRANT CONNECT ON DATABASE %I TO contacts_service', current_database());
END $$;
