...
| Indicador | Objetivo inicial |
|---|---|
| Parche de seguridad crítico de un motor, en producción | 72 horas o menos desde el aviso (comprometido el 2026-09-23) |
| Parche importante | 14 días o menos (comprometido el 2026-09-23) |
...
| B3 Avisos de seguridad | Parcial | La sección 9 de `UPSTREAM.md` fija de dónde sale cada motor, cómo llega un parche y los plazos, comprometidos el 2026-09-23 (72 horas críticas, 14 días altas): la incidencia mensual de `imagenes-motores.yml` escribe su fecha límite y se asigna al responsable (`MANTENIMIENTO_RESPONSABLE` o el dueño del repositorio), igual que la semanal de mailcow. Suscribirse a los avisos de cada proyecto es una acción de quien opera el servidor (P) |
| B9 Mantenimiento y estado de producción | V (2026-09-23) | `docs/Operacion_Despliegue.md`, sección 12: ritmo semanal, mensual y trimestral, responsable y orden de despliegue. `scripts/estado-produccion.sh` lee del servidor qué corre y lista lo pendiente (commits, migraciones por capa, servicios, motores y CI), reutilizando la decisión de los despliegues con `DEPLOY_PLAN=1`; cada despliegue deja una línea en `.deploy-log` del servidor. Probado sin docker con un servidor simulado por `ssh` (`ops/scaffold/check-estado-produccion.sh`, en `validate.sh`, con una mutación) y con docker contra un servidor real simulado (`ops/scaffold/test-deploy-mail.sh`) |
...
| A4 Ajuste del antispam | Parcial | V (2026-09-21): `mail_security_quarantine_messages_total{outcome}` (retenidos, liberados, descartados y entrenados como spam) con pruebas unitarias, para medir falsos positivos, y `docs/Ajuste_Antispam.md` con la regla (ningun peso sin dato), las consultas, el registro de cambios y las condiciones previas de DQS. No arranca el ajuste hasta que haya trafico real (P). Hecho tambien el aprendizaje contrario: "Liberar y marcar como legitimo" (`release-ham`, `learned_ham`), con permisos de liberar y de entrenar. V (2026-09-23): el aprendizaje tiene su propia contrasena del controller, `RSPAMD_CONTROLLER_ENABLE_PASSWORD`, distinta de la de lectura de la pantalla Antispam (`RSPAMD_CONTROLLER_PASSWORD`), las dos en el almacen de secretos y con sus hashes generados en el arranque de Rspamd (ADR 0009, seccion 4): Rspamd 4.1.4 deja que la contrasena de lectura escriba si falta la de escritura. Activar el aprendizaje en produccion es poner esa segunda contrasena (P) |
